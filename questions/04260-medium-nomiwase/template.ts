type StringToUnion2<S> = S extends `${infer F}${infer R}`
  ? F | StringToUnion2<R>
  : S
type AllCombinations<S extends string, T extends string = StringToUnion2<S>
, U extends string = T> =
...
// 我们需要在递归中保存联合类型，因此需要添加一个参数 T，默认值为 StringToUnion<S>。
// 我们还需要将联合类型分离，因而再添加一个参数 U，默认值为 T。
...
// 现在这段代码无论传什么都会返回 never，因为最后联合类型 T 中的所有类型都
// 被删去了，但我们的逻辑是正确的，现在就保持这样。接下来我们考虑空字符串 ''，
// 我们可以将 ''，'A'，'AB' 这样的结果视作字母与多个空字符串传的组合，换言之，
// 空字符串在排列组合中可以出现多次。
...
// 最后，我们需要想想怎么让我们的的类型跳出递归。现在，由于空字符串不会被从 T 中删去，
// 这段代码会无限递归，而且，我们也无法通过 T 来判断是否应当跳出递归。那么我们还能通过什么来判断呢？
